...
export function getBridgePath() {
    return join(import.meta.dirname ?? ".", "..", "bridge", "target", "release", "keychat-openclaw");
}
export async function ensureBinary() {
    ...
    if (!artifact) {
        throw new Error(`No pre-compiled keychat-bridge binary for ${key}. ` +
    ...
    try {
        ...
        throw new Error(`Failed to download bridge binary: ${err.message}\n` +
            `Build from source: cd bridge && cargo build --release`);
    }
}
