 ...
 */
export function checkKeychainAvailable() {
    try {
 ...
 */
export function autoFixKeychain(log) {
...
export async function storeMnemonic(accountId, mnemonic) {
...
export async function retrieveMnemonic(accountId) {
    const key = `mnemonic-${accountId}`;
    try {
        if (process.platform === "darwin") {
...
export async function deleteMnemonic(accountId) {
    const key = `mnemonic-${accountId}`;
    try {
        if (process.platform === "darwin") {
            execFileSync("security", [
