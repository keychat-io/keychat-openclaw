...
function resolveKctype(filePath, mimeType) {
 ...
 */
async function uploadToS3Relay(encrypted, hash, server) {
    ...
    const params = (await presignResponse.json());
    console.log(`[keychat] S3 presign raw: url=${params.url} access_url=${params.access_url}`);
    if (!params.url)
        throw new Error("S3 relay presign response missing url");
    if (!params.access_url)
        throw new Error("S3 relay presign response missing access_url");
 ...
 */
async function uploadToBlossom(encrypted, hash, signEvent, server) {
    const baseUrl = server || DEFAULT_MEDIA_SERVER;
    ...
    if (!data.url)
        throw new Error("Blossom upload response missing url");
    return data.url;
}
/**
 * Detect whether a server supports S3 relay upload by checking /api/v1/info.
 */
async function isS3Relay(server) {
    try {
 ...
 */
export async function encryptAndUpload(filePath, signEvent, server, mimeType, voiceNote) {
    ...
    if (voiceNote || kctype === "voiceNote") {
        ...
        if (voiceNote?.duration)
            mediaUrl.searchParams.set("duration", voiceNote.duration.toString());
        if (voiceNote?.waveform)
            mediaUrl.searchParams.set("waveform", voiceNote.waveform);
    }
    return { mediaUrl: mediaUrl.toString(), kctype };
}
/** Parse a Keychat encrypted media URL. Returns null if not a media message. */
export function parseMediaUrl(content) {
...
export async function downloadAndDecrypt(media) {
