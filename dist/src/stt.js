...
async function findWhisperBinary(configPath) {
...
async function findModel(configPath, modelSize) {
    if (configPath && existsSync(configPath))
        return configPath;
    ...
    throw new Error(`Whisper model ggml-${size}.bin not found. Download it:\n` +
 ...
 */
export async function transcribeLocal(audioPath, config = { provider: "whisper-cpp" }) {
 ...
 */
export async function transcribeOpenAI(audioPath, apiKey, language) {
 ...
 */
export async function transcribe(audioPath, config = { provider: "whisper-cpp" }) {
    if (config.provider === "openai") {
        if (!config.openaiApiKey)
            throw new Error("OpenAI API key required for openai STT provider");
